...
for (const locale of ['ja', 'en'] as const) {
  test(`${locale}: compact forms preserve controls and keep adjacent sections close`, async ({ page, request }) => {
    ...
    for (const width of [320, 390, 1440]) {
      ...
      await expect(page.locator('[data-calendar-date]').first()).toHaveCSS('height', locale === 'en' && width <= 360 ? '32px' : '36px')
    ...
    for (const width of [320, 390, 1440]) {
      ...
      await expect(form.locator('.answer-inputs button').first()).toHaveCSS('height', locale === 'en' && width <= 360 ? '28px' : '32px')
